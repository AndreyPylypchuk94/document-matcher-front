...
export const getWords = createAsyncThunk('Get words', async (params) => {
    const {categoryId} = params;
    const response = await getWordsAPI(categoryId);
    return response.data;
});
...
export const addWord = createAsyncThunk('Add word', async (params) => {
    const {word, regexes, id, categoryId} = params;

    const response = await addWordAPI(word, regexes, id, categoryId);
    return response.data;
});
